...
export async function GET(request: Request) {
  ...
  // 1. Accept POST requests with manually-entered eVA lead data (from bulk import)
  ...
  try {
    await supabase.from('state_feed_logs').insert({
      ...
      details: {
        message: 'eVA feed is ready for bulk import. Use POST endpoint or import component to add leads.',
        recent_leads_count: recentEvaLeads?.length ?? 0,
        dryRun,
      },
    })
  } catch { /* non-fatal */ }
  ...
  return NextResponse.json({
    ...
    feed: 'eva',
    dryRun,
    message: 'eVA feed is operational. Manual import via POST or UI component required.',
    inserted,
    updated,
    skipped,
...
// ── POST endpoint for bulk importing eVA leads ─────────────────────────────
export async function POST(request: Request) {
  ...
  for (const opp of leads) {
    ...
    if (dryRun) {
      inserted += targetEntities.length
      continue
    }
    ...
    for (const entity of targetEntities) {
      ...
      try {
        ...
        if (lookupErr) {
          errorLog.push(`lookup(eva/${entity}/${stateProcurementId}): ${lookupErr.message}`)
          continue
        }
        ...
        if (existing) {
          ...
          if (updateErr) {
            errorLog.push(`update(eva/${entity}/${stateProcurementId}): ${updateErr.message}`)
          } else {
            updated++
          }
        } else {
          ...
          if (insertErr) {
            errorLog.push(`insert(eva/${entity}/${stateProcurementId}): ${insertErr.message}`)
          } else {
            inserted++
          }
        }
      } catch (e) {
        errorLog.push(`exception(eva/${entity}/${stateProcurementId}): ${String(e)}`)
      }
    }
  }
