...
export function exportLeadsToCSV(leads: GovLead[], categories: ServiceCategory[]): number {
  ...
  // BOM (﻿) + UTF-8 charset in MIME type forces Excel to interpret
  // the file as UTF-8 instead of defaulting to Windows-1252. Without this,
  // every em dash, check mark, or accented char becomes mojibake (â€", â, Â).
  const bom = '﻿'
