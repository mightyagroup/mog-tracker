// Minimal Google Drive client that uses a service account JSON to sign a JWT
// and obtain an OAuth access token, then calls the Drive REST API. Avoids the
// heavy `googleapis` package by doing JWT + fetch directly.
...
export async function listFilesInFolder(sa: ServiceAccountJson, folderId: string): Promise<{ files: Array<{ id: string; name: string; mimeType: string }>; count: number }> {
...
export async function createFolder(sa: ServiceAccountJson, parentId: string, name: string): Promise<{ id: string; name: string; webViewLink: string }> {
  const token = await getAccessToken(sa)
...
/** Upload a file to a Drive folder. Uses multipart upload. */
export async function uploadFile(sa: ServiceAccountJson, folderId: string, filename: string, mimeType: string, content: Buffer): Promise<{ id: string; name: string; webViewLink: string }> {
  const token = await getAccessToken(sa)
